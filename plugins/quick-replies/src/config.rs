...
//
// Quick Replies itself needs **no** privileged bit — it only *reads* a guild's
// role list for the gate picker (any member of the guild can do that; even that
// is optional). But its invite must still request the full shared union so
// re-inviting through *this* plugin's link can't strip self-role's or tickets'
// grants. This MUST mirror `SHARED_BOT_PERMISSIONS` in the DWEEB frontend
// (`src/core/guild/config.ts`) and the constant in the other plugins. Bump all
// together when a plugin's needs change.
...
#[derive(Clone)]
pub struct Config {
    /// Port to bind. Defaults to 8096.
    pub port: u16,
    /// Public origin this service is reachable at, e.g.
    /// `https://quickreplies.example.com`. Used to build the `configUrl` in the
    ...
    /// SQLite database file path. Defaults to `./quick-replies.db`.
    pub database_path: String,
    /// The deployment-wide shared bot token. OPTIONAL here: a quick reply is a
    /// pure config-blob → pick → reply, so the click path never touches Discord.
    /// This token powers exactly one config-time convenience — listing a guild's
    /// roles in the role-gate picker so an admin picks a role by name instead of
    /// pasting an id. None = role-gating can't be set up and the UI says so;
    /// everything else still works. Stored only in memory, never returned to a
    /// browser.
...
impl Config {
    pub fn from_env() -> Result<Self, String> {
        ...
        let database_path =
            env::var("DATABASE_PATH").unwrap_or_else(|_| "./quick-replies.db".to_string());
    ...
    /// can list roles for the role-gate picker.
...
mod tests {
    ...
    fn rewrites_a_too_narrow_permissions_to_the_union() {
        // Quick Replies needs no privileged bit, but its invite must still carry
        // the full shared union so re-inviting through it can't strip another
        // plugin's grant.
