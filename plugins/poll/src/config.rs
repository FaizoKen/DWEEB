...
//
// Poll itself needs **no** privileged bit — listing roles for the vote-gate
// picker requires none. But its invite must still request the full shared union
// so re-inviting through *this* plugin's link can't strip self-role's or
// tickets' grants. This MUST mirror `SHARED_BOT_PERMISSIONS` in the DWEEB
// frontend (`src/core/guild/config.ts`) and the constant in the other plugins.
// Bump all together when a plugin's needs change.
...
#[derive(Clone)]
pub struct Config {
    /// Port to bind. Defaults to 8098.
    pub port: u16,
    /// Public origin this service is reachable at, e.g.
    /// `https://poll.example.com`. Used to build the `configUrl` in the
    ...
    /// SQLite database file path. Defaults to `./poll.db`.
    pub database_path: String,
    /// The deployment-wide shared bot token. OPTIONAL here: a poll runs its
    /// whole lifecycle off interaction responses, so this is needed only to
    /// list a guild's roles in the config UI's vote-gate / host-role pickers.
    /// None = that picker is disabled and the UI says so; everything else
    /// still works. Stored only in memory, never returned to a browser.
...
impl Config {
    pub fn from_env() -> Result<Self, String> {
        ...
        let database_path = env::var("DATABASE_PATH").unwrap_or_else(|_| "./poll.db".to_string());
    ...
    /// can list roles for the vote gate / host-role pickers.
...
mod tests {
    ...
    fn rewrites_a_too_narrow_permissions_to_the_union() {
        // Poll needs no privileged bit, but its invite must still carry the
