...
async fn run() {
    ...
    // One shared client with a bounded idle pool. The 2.2s timeout keeps a reply
    ...
    // exception. It is deliberately SHORTER than the dispatcher's 2.5s forward
    // budget: both were 2.5s, and since this clock starts later the outer one
    // always expired first — a slow Discord then surfaced as the dispatcher's
    // nonspecific "the plugin didn't respond" plus a paging alert, instead of
    // this plugin's own accurate reply. Keep it under FORWARD_BUDGET
