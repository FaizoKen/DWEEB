...
| **Edit in DWEEB** | message menu | Share link opening the editor pre-loaded with the message. The resolved payload from the interaction is re-encoded into the same `#s=<version>.<lz-string>` token `encodeShare` emits (`src/core/serialization`), so the deployed frontend needs no changes — and the message rides the URL fragment, which never reaches a server. Legacy messages are converted to Components V2 so *any* message opens editable: `content` becomes a leading Text Display, rich embeds become Containers (color → accent, title/description/fields → text, thumbnail → section accessory, image → gallery; lossy, and noted in the reply), and image/video attachments become a Media Gallery. Auto-generated link previews are skipped — they regenerate from the text. The link also carries the message's *origin* in the hash (all public ids — no webhook **token** ever travels): `&g=` guild id for any guild message, and, for a webhook message, `&w=` webhook id + `&m=` message id (+ `&t=` thread id when threaded). The editor uses these to (a) **switch to that server** so its roles/channels/emojis resolve (handed to the account menu's auto-connect, so it lands once the user is signed in), and (b) **update the original in place** when that webhook is saved in the browser — otherwise it prefills the Restore panel with the message id so the user adds the webhook once. If the message is too large to fit in the reply link, it falls back to a **short link** (`/s/<id>`) minted on the proxy (`SHORTLINK_API`), with the origin riding in its fragment so update-in-place and server targeting survive. |
| **Export JSON** | message menu | The postable wire JSON in a code block; falls back to an editor link when it outgrows the 2000/4000-char reply budget, and to a short link when even that link is too large. |
