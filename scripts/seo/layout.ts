...
export function renderTemplatePage(
  ...
  // The template as the two bot libraries receive it, generated by the same
  // code the app's Code tab runs. A developer who searched for this message by
  // name ("welcome message discord.js") lands here, and JSON alone would send
  // them away to retype it. cURL, fetch and Python are one click further, in
  // the editor, to keep the page itself lean.
  const codeUrl = withClientParams(`${SITE.origin}/#template=${encodeURIComponent(seo.id)}`, {
    intent: "code",
    entry: `template:${seo.slug}`,
  });
  const codeSection = code.length
    ? `<section class="block prose" id="template-code">
    <h2>Use this template in a bot</h2>
    <p>The same message as a <a href="/guides/discord-js-components-v2/">discord.js</a> or <a href="/guides/discord-py-components-v2/">discord.py</a> bot sends it. Both are generated from the template above by DWEEB's <a href="/features/discord-code-generator/">code generator</a>; for a cURL, JavaScript or Python webhook request, <a href="${attr(codeUrl)}" data-analytics="template" data-analytics-id="${attr(seo.slug)}" data-analytics-location="body">open the template with the Code tab</a>.</p>
    ${code
      .map(
        (item) =>
          `<details class="faq-item"><summary>${escapeHtml(item.label)} code</summary><pre class="code-block" tabindex="0" aria-label="${attr(item.label)} code for this template"><code>${escapeHtml(item.code)}</code></pre></details>`,
      )
      .join("")}
  </section>`
    : "";
