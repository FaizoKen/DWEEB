  ...
  plugins: [
    ...
    VitePWA({
      ...
      workbox: {
        ...
        //
        // The final rule denies any path whose last segment has an extension
        // (`/sitemap.xml`, `/robots.txt`, `/privacy.html`, …): a top-level hit on
        // one of those is a *navigation*, so without this the fallback hands the
        // visitor index.html and the file appears to render as the homepage. App
        // routes and base62 short-link ids never contain a dot, so nothing that
        // needs the shell is caught by it.
