 ...
 *
 * The gallery renders this array as-is (no sorting) and the unfiltered Template
 * tab opens on the first rows, so the order is curated by *how much the card
 * shows*: image galleries and thumbnails first, then multi-plugin and
 * interactive panels, then text-only cards. The 15 RoleLogic link templates
 * (`verify` plus the Roles category) run last as one family — every one of them
 * renders the same container + single link button, so leading with them filled
 * the opening screen with near-identical cards. Slot new entries by that rule
 * rather than appending, and keep `showcase` at index 0 (`data/showcase.ts`
 * mirrors it as the default message).
 */
